# NOTE: Some structure and starter code was provided, but most
# of the implementations were done from scratch. This project
# uses Python 2.7.
...
class RegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.w1 = Variable(1,75)
        self.b1 = Variable(75)
        self.w2 = Variable(75,75)
        self.b2 = Variable(75)
        self.w3 = Variable(75,1)
        self.b3 = Variable(1)
    ...
    def run(self, x, y=None):
        ...
        """
        graph = Graph([self.w1, self.b1, self.w2, self.b2, self.w3, self.b3])
        input_x = Input(graph, x)
        xw = MatrixMultiply(graph, input_x, self.w1)
        xw_plus_b = MatrixVectorAdd(graph, xw, self.b1)
        relu_xw_plus_b = ReLU(graph, xw_plus_b)
        reluw = MatrixMultiply(graph, relu_xw_plus_b, self.w2)
        reluw_plus_b2 = MatrixVectorAdd(graph, reluw, self.b2)
        relu2 = ReLU(graph, reluw_plus_b2)
        reluw3 = MatrixMultiply(graph, relu2, self.w3)
        reluw3_plus_b3 = MatrixVectorAdd(graph, reluw3, self.b3)
        ...
        if y is not None:
            ...
            loss = SquareLoss(graph, reluw3_plus_b3, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            return graph.get_output(reluw3_plus_b3)
...
class OddRegressionModel(Model):
    ...
    def __init__(self):
        ...
        self.w1 = Variable(1,100)
        self.b1 = Variable(100)
        self.w2 = Variable(100,1)
        self.b2 = Variable(1)
    ...
    def run(self, x, y=None):
        ...
        Note: DO NOT call backprop() or step() inside this method!
        """
        graph = Graph([self.w1, self.b1, self.w2, self.b2])
        input_x = Input(graph, x)
        input_neg_one = Input(graph, np.array([[-1.0]]))

        xw = MatrixMultiply(graph, input_x, self.w1)
        xw_plus_b = MatrixVectorAdd(graph, xw, self.b1)
        relu_xw_plus_b = ReLU(graph, xw_plus_b)
        reluw = MatrixMultiply(graph, relu_xw_plus_b, self.w2)
        f_pos = MatrixVectorAdd(graph, reluw, self.b2)

        neg_x = MatrixMultiply(graph, input_x, input_neg_one)
        neg_xw = MatrixMultiply(graph, neg_x, self.w1)
        neg_xw_plus_b = MatrixVectorAdd(graph, neg_xw, self.b1)
        neg_relu_xw_plus_b = ReLU(graph, neg_xw_plus_b)
        neg_reluw = MatrixMultiply(graph, neg_relu_xw_plus_b, self.w2)
        f_neg = MatrixVectorAdd(graph, neg_reluw, self.b2)
        neg_f_neg = MatrixMultiply(graph, f_neg, input_neg_one)

        f_final = Add(graph, f_pos, neg_f_neg)
        # relu2 = ReLU(graph, reluw_plus_b2)
        # reluw3 = MatrixMultiply(graph, relu2, self.w3)
        # reluw3_plus_b3 = MatrixVectorAdd(graph, reluw3, self.b3)
        ...
        if y is not None:
            ...
            # added to the graph.
            input_y = Input(graph, y)
            loss = SquareLoss(graph, f_final, input_y)
            return graph
        else:
            # At test time, the correct output is unknown.
            # You should instead return your model's prediction as a numpy array
            return graph.get_output(f_final)
...
class DigitClassificationModel(Model):
    ...
    def __init__(self):
        ...
        self.w1 = Variable(784,150)
        self.b1 = Variable(150)
        self.w2 = Variable(150,150)
        self.b2 = Variable(150)
        self.w3 = Variable(150,10)
        self.b3 = Variable(10)
    ...
    def run(self, x, y=None):
        ...
        if y is not None:
            input_y = Input(graph, y)
            loss = SoftmaxLoss(graph, reluw3_plus_b3, input_y)
            return graph
        else:
            return graph.get_output(reluw3_plus_b3)
...
class DeepQModel(Model):
    ...
    def __init__(self):
        ...
        self.w1 = Variable(4,100)
        self.b1 = Variable(100)
        self.w2 = Variable(100,100)
        self.b2 = Variable(100)
        self.w3 = Variable(100,2)
        self.b3 = Variable(2)
    ...
    def run(self, states, Q_target=None):
        ...
        if Q_target is not None:
            input_q = Input(graph, Q_target)
            loss = SquareLoss(graph, reluw3_plus_b3, input_q)
            return graph
        else:
            return graph.get_output(reluw3_plus_b3)
...
class LanguageIDModel(Model):
    ...
    def __init__(self):
        ...
        # You may use any learning rate that works well for your architecture
        self.learning_rate = .2
        self.w1 = Variable(self.num_chars,150)
        self.b1 = Variable(150)
        self.w2 = Variable(16,150)
        self.b2 = Variable(len(self.languages))
        self.w3 = Variable(150,len(self.languages))
    ...
    def run(self, xs, y=None):
        ...
        for cha in xs:
            graph = Graph([self.w1, self.b1, self.w2, self.b2, self.w3])
            input_x = Input(graph, cha)
            input_h = Input(graph, h)
            xw = MatrixMultiply(graph, input_x, self.w1)
            hw = MatrixMultiply(graph, input_h, self.w2)
            ww = Add(graph, xw, hw)
            ww_plus_b = MatrixVectorAdd(graph, xw, self.b1)
            relu_ww_plus_b = ReLU(graph, ww_plus_b)
            reluw = MatrixMultiply(graph, relu_ww_plus_b, self.w3)
            reluw_plus_b2 = MatrixVectorAdd(graph, reluw, self.b2)
            h = reluw_plus_b2
