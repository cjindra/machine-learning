...
class Graph(object):
    ...
    def get_output(self, node):
        ...
        """
        return self.outs[self.nodes.index(node)]
    ...
    def get_gradient(self, node):
        ...
        """
        return self.grads[self.nodes.index(node)]
    ...
    def add(self, node):
        ...
        out = node.forward(self.get_inputs(node))
        self.outs.append(out)
        self.grads.append(np.zeros_like(out))
    ...
    def backprop(self):
        ...
        lnGrad = self.get_gradient(loss_node)
        lnGrad = 1.0
        lparGrad = self.get_gradient(loss_node)
        lparGrad += lnGrad
        for nd in self.get_nodes()[::-1]:
            backout = nd.backward(self.get_inputs(nd), self.get_gradient(nd))
            i = 0
            for par in nd.get_parents():
                parGrad = self.get_gradient(par)
                parGrad += backout[i]
                i+= 1
    ...
    def step(self, step_size):
        ...
        for nd in self.nodes:
            if isinstance(nd, Variable):
                nd.data -= step_size * self.get_gradient(nd)
...
class ReLU(FunctionNode):
    ...
    @staticmethod
    def forward(inputs):
        return np.where(inputs[0] >= 0, inputs[0], np.zeros(inputs[0].shape))

    @staticmethod
    def backward(inputs, gradient):
        return [np.where(inputs[0] >= 0, gradient, np.zeros(inputs[0].shape))]
...
class SquareLoss(FunctionNode):
    ...
    @staticmethod
    def backward(inputs, gradient):
        return [gradient*(inputs[0]-inputs[1])/inputs[0].size, gradient*(inputs[1]-inputs[0])/inputs[1].size]
